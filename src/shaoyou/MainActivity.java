...
public class MainActivity extends BaseActivity implements OnClickListener,
	...
	private final String mAqiShareStr = "空气质量指数(AQI):%s μg/m³" + "\n"
			+ "等级[%s];" + "\n" + "PM2.5浓度值:%s μg/m³。" + "\n" + "建议：" + "%s "
			+ "\n";// aqi、等级、pm2.5、建议
	private final String mShareEndStr = "（天气渐冷，请您注意保暖，不要着凉·~(*^__^*) ）";
	private final String downString = "\n"
			+ "下载云想："
			+ "地址一：https://github.com/qin-shaoyou/BeiFen/blob/master/Android/%E6%88%91%E7%9A%84App/YX.apk?raw=true"
	...
	private boolean nimabidewangluol = false;
	...
	class NetworkChangeReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (networkInfo != null && networkInfo.isAvailable()) {
				nimabidewangluol = true;
			} else {

				Toast.makeText(context, "-_-。sorry！您的网络连接(不可用)",
		...
		if (nimabidewangluol) {
			...
			new AsyncTask<Void, Void, File>() {
				...
				protected void onPostExecute(File result) {
					...
					String time = TimeUtils.getDateTime(System
							.currentTimeMillis());
					...
					String shareStr = mShareNormalStr + mAqiShareStr
							+ mShareEndStr;
					...
					intent.putExtra("sms_body", shareStr + downString);
	...
	private void shareTo() {
		if (nimabidewangluol) {
			...
			String time = ":"
					+ TimeUtils.getDateTime(System.currentTimeMillis()) + "\n";
			String name = mFragmentAdapter.getPageTitle(
					mMainViewPager.getCurrentItem()).toString();
			...
			if (info.getAqi() == null || info.getAqi().getAqi() < 0) {
				shareStr = mShareNormalStr + mShareEndStr;
				shareStr = String.format(shareStr, new Object[] { timestr,
						name, weather, temp });
			} else {
				shareStr = String.format(shareStr, new Object[] { timestr,
						name, weather, temp, info.getAqi().getAqi(),
			...
			intent.putExtra("sms_body", shareStr + downString);
			intent.putExtra("android.intent.extra.TEXT", shareStr + downString);
