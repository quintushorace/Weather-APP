...
public class ManagerCityActivity extends BaseActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences share = getSharedPreferences("huanying2",
				Activity.MODE_WORLD_READABLE);

		String str = share.getString("yesorno", "");
		if (!str.equals("yes")) {

			Toast t = Toast.makeText(ManagerCityActivity.this,
					"长按城市可进入编辑模式~.~",
					Toast.LENGTH_LONG);
			t.setGravity(Gravity.CENTER, 0, 0);
			t.show();
			execToast(t);
			Toast.makeText(ManagerCityActivity.this, "长按城市可进入编辑模式~(☆_☆)", 1)
					.show();

		}

		// Toast.makeText(ManagerCityActivity.this, "长按城市可进入编辑模式~", 1).show();
		...
		mGridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
					long arg3) {
				// Toast.makeText(ManagerCityActivity.this, arg2 + "",
				// 0).show();
				// Intent intent = new Intent(); // Activity1

				// intent.putExtra("one", arg2 + "");

				// intent.setClass(ManagerCityActivity.this,
				// MainActivity.class);
				...
				SharedPreferences sharedPreferences = getSharedPreferences(
						"chuandi_city", Context.MODE_PRIVATE); // 私有数据
				Editor editor = sharedPreferences.edit();// 获取编辑器

				editor.putInt("city_id", arg2);
				editor.commit();// 提交修改
	...
	private final DragSortGridView.OnReorderingListener dragSortListener = new DragSortGridView.OnReorderingListener() {
	...
	private void getWeather(City city) {
		...
		StringRequest sr = new StringRequest(String.format(
				WeatherSpider.WEATHER_ALL, postID),
			...
			StringRequest sr = new StringRequest(String.format(
					...
					new Response.Listener<String>() {
						...
						public void onResponse(String response) {
							try {
								WeatherInfo weatherInfo = WeatherSpider
										.getWeatherInfo(mActivity, postID,
												response);
								if (!WeatherSpider.isEmpty(weatherInfo)) {
									save2Database(postID, response, weatherInfo);
									mTmpCitys = getTmpCities();
